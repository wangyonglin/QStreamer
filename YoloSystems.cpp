...
YoloSystems::AudioPlayer::AudioPlayer(int sample_rate, SDL_AudioFormat sample_fmt, int channels)
    :__sample_rate(sample_rate),__sample_fmt(sample_fmt),__channels(channels)
{

}

YoloSystems::AudioPlayer::~AudioPlayer()
{

}

int YoloSystems::AudioPlayer::initAudioPlayer()
{
...
void YoloSystems::AudioPlayer::exitAudioPlayer()
{

}

int YoloSystems::AudioPlayer::open()
{
...
int YoloSystems::AudioPlayer::addQueueAudioValue(const void *data, Uint32 dst_datasize,bool waitTime)
{
    int error=-1;
...
void YoloSystems::AudioPlayer::pause(int time)
{
    SDL_PauseAudioDevice(__deviceId, time);
}

int YoloSystems::AudioPlayer::close()
{
    if(__deviceId){
...
Uint32 YoloSystems::AudioPlayer::getQueuedAudioSize()
{
    if(__deviceId){
...
SDL_AudioSpec YoloSystems::AudioPlayer::getObtained()
{
    return obtained;
}
...
#if USE_WINDOW
YoloSystems::VideoPlayer::VideoPlayer(QOpenGLWindow::UpdateBehavior updateBehavior, QWindow *parent):QOpenGLWindow(updateBehavior, parent)
{
...
#else
YoloSystems::VideoPlayer::VideoPlayer(QWidget *parent, Qt::WindowFlags f): QOpenGLWidget(parent, f)
{
    // 初始化视图大小，由于Shader里面有YUV转RGB的代码，会初始化显示为绿色，这里通过将视图大小设置为0避免显示绿色背景
...
YoloSystems::VideoPlayer::~VideoPlayer()
{
...
void YoloSystems::VideoPlayer::repaint(AVFrame *frame)
{
    // 如果帧长宽为0则不需要绘制
...
void YoloSystems::VideoPlayer::initializeGL()
{
...
void YoloSystems::VideoPlayer::resizeGL(int w, int h)
{
...
void YoloSystems::VideoPlayer::paintGL()
{
